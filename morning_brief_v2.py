...
def call_gemini(prompt):
    ...
    payload = {"contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.3, "maxOutputTokens": 1200}}
...
def call_deepseek(system, user):
    payload = {"model": "deepseek-chat", "max_tokens": 600, "temperature": 0.2,
...
def get_cs153_video():
    ...
    print(f"Latest video: {video_id} | {title[:60]}")
    return {"title": title, "video_id": video_id, "url": f"https://www.youtube.com/watch?v={video_id}",
...
def get_transcript(video_id):
    ...
    try:
        ...
        text = " ".join(c["text"] for c in chunks)[:12000]
...
h2{{font-size:17px;margin-top:32px;border-left:3px solid #6366f1;padding-left:10px}}
...
def main():
    ...
    print("\n--- EMAIL ---")
    send_email(f"☀️ Morning Brief — {today.strftime('%A, %B %d, %Y')}",
